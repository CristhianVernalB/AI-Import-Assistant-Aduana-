...
class SessionManager:
    def __init__(self):
        ...
        print("DEBUG → DATABASE_URL =", self.database_url)
    ...
    def _initialize_database(self):
        ...
        # ... (Código _initialize_database sin cambios) ...
        create_tables_sql = """
    ...
    def get_saved_sessions(self, limit: int = 50) -> List[Dict[str, Any]]:
        # ... (Código get_saved_sessions sin cambios) ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
    ...
    def delete_session(self, session_id: str) -> bool:
        # ... (Código delete_session sin cambios) ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
    ...
    def update_session_notes(self, session_id: str, user_notes: str) -> bool:
        # ... (Código update_session_notes sin cambios) ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
    ...
    def get_session_statistics(self) -> Dict[str, Any]:
        # ... (Código get_session_statistics sin cambios) ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
    ...
    def render_session_manager(self):
        # ... (Código render_session_manager sin cambios) ...
    ...
    def _render_save_session(self):
        ...
        if not st.session_state.get('validated_data'):
            st.warning("⚠️ No hay datos validados para guardar. Realice la Extracción y Validación primero.")
            return
    ...
    def _render_load_session(self):
        # ... (Código _render_load_session sin cambios, solo las referencias a keys) ...
        ...
        for session in sessions:
            ...
            # Se elimina la llamada a len() que causaba el error
            with st.expander(
                ...
                with col3:
                    if st.button(f"🗑️ Eliminar", key=f"delete_{session['id']}", use_container_width=True):
                        ...
                        if st.session_state.get(f"confirm_delete_{session['id']}", False):
                            ...
                            st.button(f"⚠️ Confirmar Eliminación", key=f"confirm_delete_btn_{session['id']}", type="secondary")
    ...
    def _delete_session_action(self, session_id: str):
        # ... (Código _delete_session_action sin cambios) ...
        with st.spinner("🗑️ Eliminando sesión..."):
    ...
    def _render_session_statistics(self):
        # ... (Código _render_session_statistics sin cambios) ...
