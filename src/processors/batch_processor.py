...
class BatchProcessor:
    ...
    def _render_current_job_progress(self):
        ...
        if job.status == 'pending':
            ...
            if job.status == 'completed':
                ...
                st.warning(f"⚠️ Lote '{job.name}' fue cancelado")
    ...
    def _render_job_card(self, job: BatchJob):
        ...
        with st.expander(f"{status_emoji} {job.name} - {job.status.title()}", expanded=False):
            ...
            with col2:
                if job.status == 'running':
                    if st.button(f"⏹️ Cancelar", key=f"cancel_{job.id}", use_container_width=True):
                        self.cancel_job(job.id)
                        st.rerun()
            
            with col3:
                if job.status in ['completed', 'failed', 'cancelled']:
                    if st.button(f"🗑️ Eliminar", key=f"delete_{job.id}", use_container_width=True):
